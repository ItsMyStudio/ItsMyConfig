...
public class ConsoleFilter extends AbstractFilter {
    ...
    private Result checkMessage(Message message) {
        ...
        if (parsed.isPresent()) {
            ...
            // Send the formatted message to the console sender
            // We use AudienceResolver to leverage Adventure's handling of components
            AudienceResolver.resolve(Bukkit.getConsoleSender()).sendMessage(translated);
